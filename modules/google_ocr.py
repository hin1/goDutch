...
def get_raw_txt(file_name):
    ...
    #Label detection
    response = client.text_detection(image=img)
    json_response = MessageToDict(response,preserving_proto_field_name = True)
    
    texts = json_response["text_annotations"]
    full_text = json_response["full_text_annotation"]
    #texts = response.text_annotations
